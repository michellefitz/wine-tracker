import RefreshFactsButton from "@/components/RefreshFactsButton";
  ...
  return (
    <section className="mx-auto mt-12 max-w-md border-t border-rule pt-7">
      <div className="mb-5 flex items-baseline justify-between gap-4">
        ...
        <RefreshFactsButton wineId={wineId} />
      </div>
      ...
      {empty ? (
        ...
        <div className="space-y-6">
          {facts.summary && (
            <p className="essay text-[1.0625rem] leading-[1.55] text-ink">{facts.summary}</p>
          )}

          {facts.style && (
            <p className="text-[0.9375rem] leading-relaxed text-ink-soft">{facts.style}</p>
          )}
            ...
            <ul className="border-t border-rule">
              {facts.details.map((detail) => (
                <li
                  ...
                  className="flex items-baseline justify-between gap-4 border-b border-rule py-3"
                >
                  <span className="eyebrow pt-0.5">{detail.label}</span>
                  <span className="text-right text-[0.9375rem] tabular-nums text-ink">
                    {detail.value}
                  </span>
                </li>
              ))}
